...
## Bank Services — Clean Architecture

Each bank service (`Bank1.Service`, `Bank2.Service`) is an independent bounded context:

```
Bank*.Service (API)
    ↓
Bank*.Service.Application (handlers, validators)
    ↓
Bank*.Service.Domain (entities, rules)
Bank*.Service.Infrastructure (EF Core, proxy, audit)
Bank*.Service.Contracts (API DTOs)
```

| Isolation rule | Enforcement |
|---|---|
| Separate business DB per bank | `Bank1DbContext` / `Bank2DbContext` |
| Separate audit DB per bank | `Bank1AuditDbContext` / `Bank2AuditDbContext` |
| No cross-bank references | Architecture + isolation tests |
| No bank logic in Gateway.Framework | Project references |

See [bank-service-development.md](bank-service-development.md) for migrations, Docker, testing, and proxy configuration.
